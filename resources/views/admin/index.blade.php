                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($menu->image)
                                    <img style="max-height: 100px;" src="{{ asset('images/menus/' . $menu->image) }}">
                                @else
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('categories.update', $category->id) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="editCategoryName{{ $category->id }}" class="form-label">Name</label>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                <label for="menuCategory{{ $menu->id }}" class="form-label">Category</label>
                                <select class="form-select" id="menuCategory{{ $menu->id }}" name="category_id" required>
                                    @foreach($categories as $category)
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control" id="menuPrice{{ $menu->id }}" name="harga" value="{{ $menu->harga }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="menuStock{{ $menu->id }}" class="form-label">Stock</label>
                                <input type="number" class="form-control" id="menuStock{{ $menu->id }}" name="stock" value="{{ $menu->stock }}" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="menuPoints{{ $menu->id }}" class="form-label">Points</label>
                                <input type="number" class="form-control" id="menuPoints{{ $menu->id }}" name="point" value="{{ $menu->point }}" required>
                            </div>
                            <div class="mb-3">
                                <label for="menuPortion{{ $menu->id }}" class="form-label">Portion</label>
                                <select class="form-select" id="menuPortion{{ $menu->id }}" name="porsi" required>
                                    <option value="Small" {{ $menu->porsi === 'Small' ? 'selected' : '' }}>Small</option>
                                    <option value="Medium" {{ $menu->porsi === 'Medium' ? 'selected' : '' }}>Medium</option>
                                    <option value="Large" {{ $menu->porsi === 'Large' ? 'selected' : '' }}>Large</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label for="menuImage{{ $menu->id }}" class="form-label">Image</label>
                                <input type="file" class="form-control" id="menuImage{{ $menu->id }}" name="image">
                                @if($menu->image)
                    ...
                    <div class="mb-3">
                        <label for="menuDescription{{ $menu->id }}" class="form-label">Description</label>
                        <textarea class="form-control" id="menuDescription{{ $menu->id }}" name="deskripsi" rows="3">{{ $menu->deskripsi }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label for="menuNutrition{{ $menu->id }}" class="form-label">Nutrition Facts (comma separated)</label>
...
<div class="modal fade" id="editRewardModal{{ $reward->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('categories.update', $reward->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="rewardName" class="form-label">Name</label>
                        <input type="text" class="form-control" id="rewardName" name="name" value="{{ $reward->name }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="rewardPoints" class="form-label">Points Required</label>
                        <input type="number" class="form-control" id="rewardPoints" name="poin" value="{{ $reward->poin }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="rewardMenu" class="form-label">Menu Reward</label>
                        <select class="form-select" id="rewardMenu" name="menu_id" required>
                            @foreach($menus as $menu)
                            <option value="{{ $menu->id }}" {{ $reward->menu_id == $menu->id ? 'selected' : '' }}>
                                {{ $menu->name }}
                            </option>
                            @endforeach
                        </select>
